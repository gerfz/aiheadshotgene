...
export function SubscriptionScreen({ navigation }: Props) {
  ...
  const handleSubscribe = async () => {
    ...
    if (!pkg) {
      // Fallback message when RevenueCat is not configured
      Alert.alert(
        'Setup Required',
        'To enable purchases, configure your RevenueCat API keys in src/services/purchases.ts',
        [{ text: 'OK' }]
      );
      return;
    }
    ...
    if (success) {
      Alert.alert('Welcome to Pro! 🎉', 'You now have unlimited portrait generations!');
      navigation.goBack();
    }
  };
